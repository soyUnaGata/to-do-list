<template>
  <div class="select-duration w-100">
    <button
      type="button"
      class="select-duration-btn d-flex justify-content-between w-100 style-for-inputs"
      @click.stop="show = !show"
    >
      <span class="select-duration-text">{{ currentValue?.duration }}</span>
      <img
        class="select-duration-img select-duration-arrow"
        v-if="show"
        src="../assets/img/arrow-small-up.svg"
      />
      <img
        class="select-duration-img select-duration-arrow"
        v-else
  ...
  <Transition>
    <div class="select-duration-menu d-flex flex-column" v-if="show">
      <div
        class="select-duration-item"
        v-for="(dur, index) in durations"
        @click.stop="select(dur)"
      >
